...
const FileUploadZone = memo(function FileUploadZone({
  ...
  const fileList = showFileList ? (
    <div className={`${horizontal ? 'flex-1 min-w-0 max-h-20 overflow-y-auto' : compact ? 'mt-2 max-h-[80px]' : 'mt-3 h-[120px]'} overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/30`}>
      {hasFiles ? (
  ...
  return (
    <div className={`${className} ${horizontal ? 'flex gap-2 items-stretch' : ''}`}>
      {dropZone}
      {fileList}
    </div>
  );
