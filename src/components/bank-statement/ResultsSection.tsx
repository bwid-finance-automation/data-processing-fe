...
export default function ResultsSection({
  ...
  const renderBankChips = (banks: string[], struck: Set<string>, keyPrefix: string) => {
    if (banks.length === 0) {
      return (
          ...
          return (
            <span
              ...
              className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold ${
                isStruck
                  ? 'line-through bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500'
  ...
  return (
    ...
    >
      {/* Results Card */}
      <div className="bg-white dark:bg-[#222] rounded-lg shadow-lg p-4 border border-gray-200 dark:border-gray-800 flex flex-col">
        ...
        {onShowHistory && (
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-3 py-2.5 mt-2 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 border border-gray-200 dark:border-gray-700 transition-colors"
          >
      ...
      {/* Supported Banks — separate card below */}
      {showSupportedBanks && (
        <div className="p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#222] shadow-sm">
          <div className="mb-2 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <div className="h-6 w-6 rounded-md bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center">
          ...
          {isZipMode ? (
            <div className="space-y-2">
              <div>
                ...
                <div className="mt-1">
                  {renderBankChips(supportedBanks, struckBanks, 'zip-excel')}
                </div>
              </div>
              <div>
                <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                ...
                <div className="mt-1">
                  {renderBankChips(supportedBanksPDF, struckBanksPDF, 'zip-pdf')}
                </div>
              </div>
            </div>
          ) : (
            <div>
              ...
              <div className="mt-1">
                {renderBankChips(activeBanks, activeStruckBanks, 'single')}
              </div>
            </div>
          )}
        </div>
