...
const API_BASE_URL = import.meta.env.VITE_BILLING_API_URL || 'http://localhost:8000';
...
export const createSession = async () => {
  try {
    const response = await billingApi.post('/api/finance/api/v1/billing/session/create');
...
export const cleanupSession = async (sessionId) => {
  try {
    const response = await billingApi.delete('/api/finance/api/v1/billing/session/cleanup', {
      headers: {
...
export const uploadInputFile = async (sessionId, file, onProgress) => {
  try {
    ...
    const response = await billingApi.post('/api/finance/api/v1/billing/upload/input', formData, {
      headers: {
        'X-Session-ID': sessionId,
...
export const uploadMasterDataFile = async (sessionId, file, onProgress) => {
  try {
    ...
    const response = await billingApi.post('/api/finance/api/v1/billing/upload/master-data', formData, {
      headers: {
        'X-Session-ID': sessionId,
        'Content-Type': 'multipart/form-data',
      },
...
export const listInputFiles = async (sessionId) => {
  try {
    const response = await billingApi.get('/api/finance/api/v1/billing/files/input', {
      headers: {
        'X-Session-ID': sessionId,
      },
    });
...
export const listMasterDataFiles = async (sessionId) => {
  try {
    const response = await billingApi.get('/api/finance/api/v1/billing/files/master-data', {
      headers: {
        'X-Session-ID': sessionId,
      },
    });
...
export const listOutputFiles = async (sessionId) => {
  try {
    const response = await billingApi.get('/api/finance/api/v1/billing/files/output', {
      headers: {
        'X-Session-ID': sessionId,
      },
    });
...
export const processBilling = async (sessionId, inputFiles = null) => {
  try {
    const response = await billingApi.post(
      '/api/finance/api/v1/billing/process',
...
export const getSystemStatus = async (sessionId) => {
  try {
    const response = await billingApi.get('/api/finance/api/v1/billing/status', {
      headers: {
        'X-Session-ID': sessionId,
      },
    });
...
export const getMasterDataStatus = async (sessionId) => {
  try {
    const response = await billingApi.get('/api/finance/api/v1/billing/master-data/status', {
      headers: {
        'X-Session-ID': sessionId,
      },
    });
...
export const checkHealth = async () => {
  try {
    const response = await billingApi.get('/api/finance/api/v1/billing/health');
    return response.data;
  } catch (error) {
